 ...
 */
const isLoreExists = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  const validFormat = Types.ObjectId.isValid(req.params.loreId);
  const lore = validFormat
    ? await LoreCollection.findOne(req.params.loreId)
    : "";
  if (!lore) {
    res.status(404).json({
...
export { isValidLoreContent, isValidLoreTitle, isLoreExists };
