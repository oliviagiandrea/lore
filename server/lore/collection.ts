...
class LoreCollection {
   ...
   */
  static async addOne(authorId: Types.ObjectId | string, title: string, content: string): Promise<HydratedDocument<Lore>> {
    const date = new Date();
   ...
   */
  static async findOne(loreId: Types.ObjectId | string): Promise<HydratedDocument<Lore>> {
   ...
   * @return {Promise<HydratedDocument<Lore>[]>} - An array of all of the lore
   */
  static async findAllByUsername(username: string): Promise<Array<HydratedDocument<Lore>>> {
    const author = await UserCollection.findOneByUsername(username);
    return LoreModel.find({authorId: author._id}).sort({dateModified: -1}).populate('authorId');
  }
   ...
   * @return {Promise<HydratedDocument<Lore>[]>} - An array of all of the lore
   */
  static async findAllByTitle(title: string): Promise<Array<HydratedDocument<Lore>>> {
   ...
   */
  static async updateOne(loreId: Types.ObjectId | string, content: string): Promise<HydratedDocument<Lore>> {
